...
os.chdir('E:\\Research Data\\hyperspectral\\dilution\\H006\\filtered')
...
step=5
img_filt = np.zeros(img_all2.shape)
patch = img_all2[0:q,0:q,:]
...
for i in range(step,img_all2.shape[0]-q,step):
    j=0
    patch = img_all2[0+i:q+i,0+j:q+j,:]
    for k in range(patch.shape[2]):
        patch_mat[:,k] = np.reshape(patch[:,:,k],q*q)
    L,S = GoDec(patch_mat,7,4000,10,error_bound=10)
    print(i)
...
for j in range(step,img_all2.shape[1]-q,step):
    i=0
    patch = img_all2[0+i:q+i,0+j:q+j,:]
    for k in range(patch.shape[2]):
        patch_mat[:,k] = np.reshape(patch[:,:,k],q*q)
...
for i in range(step,img_all2.shape[0]-q,step):
    for j in range(step*2,img_all2.shape[1]-q,step):
        patch = img_all2[0+i:q+i,0+j:q+j,:]
        for k in range(patch.shape[2]):
            patch_mat[:,k] = np.reshape(patch[:,:,k],q*q)
        L,S = GoDec(patch_mat,7,4000,10,error_bound=10)
